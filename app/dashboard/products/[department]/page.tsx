...
async function getData(department: string | null, category: string | null, subcategories: string[] | null, skip: number = 0, query: string = "", order: string, brands: string[], statuses: string[] | undefined) {
    ...
    const resBrands = await fetch(`${process.env.URL}api/brands`, {
        cache: 'no-store',
        method: 'GET'
    });
    ...
    const resDepartment = await fetch(`${process.env.URL}api/departments/${department}`, {
        cache: 'no-store',
        method: 'GET'
    });
...
export default async function Page({
    ...
    const baseUrl = `${process.env.URL}dashboard/${department ? '/products/' + department : ''}`
